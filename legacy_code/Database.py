...
# Shout-out to you, recruiter:
# legacy code - code which I believe gives insight into
# how I structure and write my code,
# but which will be/was replaced by a better/more convenient implementation.

# e.g psycopg2 db driver replaced with sqlAlchemy ORM
...
class TransactionRepo:
    ...
    def loadEquabankStatement(self, user: User) -> list[Transaction]:
        ...
        def parseRecord(rootObj: ET.Element, XPath: str) -> str | None:
            ...
            if isinstance(foundElement, ET.Element) and isinstance(
                foundElement.text, str
            ):
        ...
        def parseDate(rootObj: ET.Element, XPath: str) -> datetime.datetime | None:
            ...
            if isinstance(foundElement, ET.Element) and isinstance(
                ...
                return datetime.datetime.strptime(foundElement.text, "%Y-%m-%d+%H:%M")
            else:
                raise FileError(
                    f"Error while reading .xml file - transaction does not have date item. Import aborted."
                )
        ...
        for file in XMLfiles:
            ...
            try:
                ...
                for rootDir in root.findall(".//nms:Ntry", namespace):
                    ...
                    tempAmount, _ = temp[-1].amount, temp[-1].currency = parseAmount(
                        rootDir, "./nms:Amt"
                    )
                    ...
                    calculatedSum += tempAmount if isinstance(tempAmount, float) else 0
            ...
            parsedSumString = parseRecord(
                root, ".//nms:TtlNtries/nms:TtlNetNtry/nms:Amt"
            )
            if isinstance(parsedSumString, str):
                ...
                raise FileError(
                    "Monthly expense summary not included in the statements."
                )
